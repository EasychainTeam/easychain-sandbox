...
describe('smartdb-orm', () => {
  ...
  before(async () => {
    ...
    let Account = app.db.define('account', require('../framework/model/account'), { timestamps: false })
  ...
  it('test replace with cache', async () => {
    let sdb = new SmartDB(app)
    await sdb.load('Account', ['str1'], ['address'])

    sdb.beginBlock()
    sdb.beginTransaction()
    assert.equal(sdb.get('Account', { address: 'address1' }), null)
    sdb.replace('Account', { str1: 'name1', address: 'address1' })
    assert.equal(sdb.get('Account', { address: 'address1' }).str1, 'name1')
    sdb.rollbackTransaction()
    assert.equal(sdb.get('Account', { address: 'address1' }), null)
    ...
    sdb.replace('Account', { str1: 'name1', address: 'address1' })
    assert.equal(sdb.get('Account', { address: 'address1' }).str1, 'name1')
    sdb.replace('Account', { str1: 'name2', address: 'address1' })
    assert.equal(sdb.get('Account', { address: 'address1' }).str1, 'name2')
    sdb.commitTransaction()
    await sdb.commitBlock()
    assert.equal(sdb.get('Account', { address: 'address1' }).str1, 'name2')

    let dbItem = await app.model.Account.findOne({ address: 'address1' })
    assert.notEqual(dbItem, null)
    assert.equal(dbItem.str1, 'name2')
  })

  it('test replace no cache', async () => {
    let sdb = new SmartDB(app)

    sdb.beginBlock()
    sdb.beginTransaction()
    assert.throws(() => sdb.get('Account', { address: 'address1' }), Error, /Model not found in cache/)
    sdb.replace('Account', { str1: 'name1', address: 'address1' })
    assert.throws(() => sdb.get('Account', { address: 'address1' }), Error, /Model not found in cache/)
    sdb.commitTransaction()
    await sdb.commitBlock()
    ...
    let dbItem = await app.model.Account.findOne({ address: 'address1' })
    assert.notEqual(dbItem, null)
    assert.equal(dbItem.str1, 'name1')
  })
