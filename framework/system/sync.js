...
private.findUpdate = function (lastBlock, peer, cb) {
	(async () => {
		try {
			...
			while (true) {
				let blocks = await PIFY(modules.blockchain.blocks.loadBlocksPeer)(lastBlock.height, peer)
				console.log('Loading ' + blocks.length + ' blocks')
				var t = await app.db.transaction()
				try {
					for (let i in blocks) {
						let b = blocks[i]
						if (b.height > lastBlock.height) {
							await modules.blockchain.blocks.processBlock(b, { noTransaction: true })
						}
					}
					await t.commit()
				} catch (e) {
				...
				lastBlock = modules.blockchain.blocks.getLastBlock()
			}
...
private.transactionsSync = function transactionsSync(cb) {
	modules.api.transport.getRandomPeer("get", "/transactions/unconfirmed", { limit: 100 }, function (err, res) {
		...
		let trs = res.body.transactions.filter((t) => !modules.blockchain.transactions.hasUnconfirmed(t.id))
		modules.blockchain.transactions.receiveTransactions(trs, cb)
	});
}
...
private.blockSync = function blockSync(cb) {
	modules.api.blocks.getHeight(function (err, height) {
		...
		if (lastBlock.pointHeight && height - lastBlock.pointHeight <= 2) {
			return cb()
		}
