...
Accounts.prototype.login = function (req, cb) {
	...
	(async () => {
		try {
			...
			var extra = await app.model.Account.findOne({ condition: { address: address } })
			var account = {
				address: address,
				publicKey: keypair.publicKey.toString('hex'),
				balances: balances,
				extra: extra,
				isDelegate: modules.blockchain.round.isDelegateAddress(address)
			}
...
Accounts.prototype.getAccount = function (req, cb) {
	if (!req.params || !req.params.address) {
		return cb('Address should not be empty');
	}
