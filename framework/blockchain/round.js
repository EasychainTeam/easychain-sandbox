...
private.signTransaction = function (roundState, transaction) {
	return modules.logic.transaction.create(transaction, roundState.keypair)
}
...
private.loop = function (point, cb) {
	...
	var roundState = private.getState(point.height);
	if (roundState === null) {
	...
	library.sequence.add(function forgeNewBlock(cb) {
		(async function () {
			try {
				var slotNumber = slots.getSlotNumber(roundState.slotTime)
				var currentSlotNumber = slots.getSlotNumber()
				if (slotNumber === currentSlotNumber) {
					await private.balanceSync(roundState.keypair)
					await private.withdrawalSync(roundState.secret)
					if (app.hooks.beforeCreateBlock) await app.hooks.beforeCreateBlock({
						height: modules.blockchain.blocks.getLastBlock().height + 1,
						pointId: point.id,
						pointHeight: point.height,
						slotTime: roundState.slotTime,
						signTransaction: private.signTransaction.bind(null, roundState),
						addTransactions: modules.blockchain.transactions.receiveTransactionsAsync
					})
					await modules.blockchain.blocks.createBlock(roundState.keypair, roundState.slotTime, point)
					var lastBlock = modules.blockchain.blocks.getLastBlock();
...
private.balanceSync = async function balanceSync(keypair) {
	...
	for (let t of mainTransactions) {
		let exists = await app.model.Deposit.exists({ srcId: t.id })
		if (!exists) {
			unprocessedTransactions.push(t)
		}
	}
	let localTransactions = unprocessedTransactions.map((mt) => {
...
private.withdrawalSync = async function withdrawalSync(secret) {
	...
	let outerTransactions = innerTransactions.filter((t) => {
		...
		let ot = easychainJS.transfer.createOutTransfer(address, app.meta.transactionId, t.id, currency, amount, secret)
		ot.signatures = []
		for (let s of app.config.secrets) {
			if (s !== secret) {
				ot.signatures.push(easychainJS.transfer.signOutTransfer(ot, s))
			}
...
Round.prototype.isDelegateAddress = function (address) {
	return private.delegates.indexOf(address) !== -1
}
...
Round.prototype.onMessage = function (query) {
	...
	if (query.topic == 'point') {
		...
		if (!private.outTransferManager.has(ot)) {
			let signature = easychainJS.transfer.signOutTransfer(out)
