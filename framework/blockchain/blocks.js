...
Blocks.prototype.setLastBlock = function (block) {
	...
	let delegatesNumber = app.meta.delegates.length
	let round = Math.floor(block.height / delegatesNumber) + (block.height % delegatesNumber > 0 ? 1 : 0);
	app.feePool.setRound(round)
}
...
Blocks.prototype.getBlocksAfter = function (req, cb) {
	(async () => {
		...
		if (!blocks || !blocks.length) return cb(null, { blocks: [] })
		...
		library.sequence.add(function receiveNewBlock(cb) {
			...
			if (block.prevBlockId == private.lastBlock.id &&
				...
				(async () => {
					...
					try {
						let unconfirmedTrs = modules.blockchain.transactions.getUnconfirmedTransactionList()
